...
## A Note on Running This on Mobile (Termux)

During testing on Android (Termux), the original defaults (100 threads,
0.5s timeout) caused false negatives when scanning `127.0.0.1` - open
ports were sometimes reported as closed. This wasn't a logic bug: a
phone's CPU can't push through 100 simultaneous connection attempts as
fast as a laptop, so connections to genuinely open ports were timing
out before completing. Lowering the defaults to 30 threads and a 1.0s
timeout resolved this. If scanning from a phone still misses ports
you know are open, try `--timeout 2 -t 10` for an even more forgiving
scan.
